/*
	...
	File:	Q-Cogo-Misc.js
	Use:	To provide miscellaneous operations for Q-Cogo, <https://www.q-cogo.com/>.
	Ver:	1.7
...
// Tests if a device uses a touch screen
// Input:  none
// Output: True if the device is a touch device, false if not
...
// Output: The resulting points matrix or the single point
	...
	var check = 0, nameError = 0, Header = 'POINT INPUT', Range, k, m, n, Index1, Index2, Asterisk, ptMtD;
...
// Format every element in point list to match values in point records, parse ranges and replace with point lists
		...
		for (var j=0; j<PtList.length; j++) {
			...
			Range = 0; prevCount = 0; k = 0; Index1 = -1; Index2 = -1; Asterisk = '';
...
// Replace empty range values with empty string flag (much longer string than legal point)
				...
				if (Range[0].search(/\+|\-|\*|\/|\,|\.|\(|\)|\[|\]|\{|\}/) >= 0 || Range[1].search(/\+|\-|\*|\/|\,|\.|\(|\)|\[|\]|\{|\}/) >= 0) {nameError = 1;}
				else {nameError = 0;}
...
// Allow point range to include numbers on either side of the desired range, not necessarily the exact range limits

				if (!nameError) {
				
// Check for exact range points
...
// If exact range points not found, sort into dummy points matrix and form range of existing points
					
					if (Index1 < 0) {
						
						ptMtD = NonMutableArray(ptMt);
						ptMtD[ptMtD.length] = [Range[0].replace(/\*/g,''), 0, 0, 0, 'blank'];
						ptMtD.sort(sortByPt);
						
						for (m=0; m<ptMtD.length; m++) {if (Range[0].replace(/\*/g,'') == ptMtD[m][0])  Index1 = m;}
					...
					if (Index2 < 0) {
						...
						ptMtD = NonMutableArray(ptMt);
						ptMtD[ptMtD.length] = [Range[1].replace(/\*/g,''), 0, 0, 0, 'blank'];
						ptMtD.sort(sortByPt);
						
						for (n=0; n<ptMtD.length; n++) {if (Range[1].replace(/\*/g,'') == ptMtD[n][0])  Index2 = n - 1;}
						
					}
				
				}

// Remove range style notation, replace with point list
				
				PtList.splice(j, 1);  j--;
				
				if (Index1 > -1 && Index2 > -1) {

					for (k=Index1; k<=Index2; k++) {PtList.splice(j + 1, 0, ptMt[k][0]);  j++;}
...
// Alert and return error if specific point not found
...
// Alert and return error if no valid points found

	if (PtList.length == 0) {
		
		if (!suppress) {qError(element, 'No valid points found!', Header, 'errormessage');}
...
// Add leading zeros in the case of DDD°M'S.SS input (single digit M or S)

	if (Value.indexOf("'") >= 0 && Value.indexOf("°") >= 0) {
		if (Value.indexOf("'") - Value.indexOf("°") < 3) {Value = Value.split("°")[0] + "°0" + Value.split("°")[1];}
		
	}
	
	if (Value.indexOf("\"") >= 0 && Value.indexOf("'") >= 0) {
		
		if (Value.indexOf("\"") - Value.indexOf("'") < 3 || (Value.indexOf('.') >= 0 && Value.indexOf('.') - Value.indexOf("'") < 3)) {Value = Value.split("'")[0] + "'0" + Value.split("'")[1];}
...
// If duplicate point detected in points area, fill in existing values for editing
		...
		if (Check == 'X' && $('#header').html() == 'Points' && element.id == 'Pt') {
...
// Calls the appropriate function in the Points screen, based on the title
// Input:  None
// Output: The appropriate function call

	if (qG('PointsTitle').innerHTML == 'Store / Edit') {SetPoint();}
	else {BulkPoint();}
...
// Edits a point list, setting multiple points to the same coordinate (one or more of X, Y, Z, and/or Desc)
// Input:  None
// Output: The updated points with appropriate checks and warning prompts

	var PtList = qG('Pt').value;  var N = qG('N').value;  var E = qG('E').value;  var Z = qG('Z').value;  var Desc = qG('Desc').value;
	
// Check that field aren't blank, as appropriate

	if (!CheckBlank(PtList, ' list of points to edit', qG('Pt'))) return;
	if (!N &&  !E &&  !Z &&  !Desc) {qError(qG('N'), 'Enter at least one value to edit', 'REQUIRED VALUE', 'errormessage'); return;}
	
// Get edit point matrix from points record and check

	if (!(PtList = GetPointList(PtList, 1, 0, 0, qG('Pt')))) {return;}

// Check all values, if entered

	N = ((N) ? ParseDecimal(N, 1, 1) : 'blank');
	E = ((E) ? ParseDecimal(E, 1, 1) : 'blank');
	Z = ((Z) ? ParseDecimal(Z, 1, 1) : 'blank');
	
	if (N != 'X' && E != 'X' && Z != 'X' && CheckDesc(Desc))  {
		
		var NDisp = N != 'blank' ? FormatDecimal(N, 0, 1) : N;
		var EDisp = E != 'blank' ? FormatDecimal(E, 0, 1) : E;
		var ZDisp = Z != 'blank' ? FormatDecimal(Z, 0, 1) : Z;
		var DescDisp = FormatString(Desc, 1, 0, 0);
		
		var Plural = ((PtList.length > 1) ? 's' : '');

		var Message = PtList.length + ' point' + Plural + ' will be edited! Values will be set to:<br><br>';
		Message += N != 'blank' ? '<span class="par-label">N:</span>' + NDisp.replace(/\s+/g,'') + '<br>' : '';
		Message += E != 'blank' ? '<span class="par-label">E:</span>' + EDisp.replace(/\s+/g,'') + '<br>' : '';
		Message += Z != 'blank' ? '<span class="par-label">Z:</span>' + ZDisp.replace(/\s+/g,'') + '<br>' : '';
		Message += Desc ? '<span class="par-label">Desc:</span>' + DescDisp.replace(/\s+/g,'') + '<br>' : '';
		Message += '<br>';
		Message += 'Edit point' + Plural + '?';
	
		$.prompt(qP(Message, 'EDIT POINTS'), {
...
// Update points matrix with new values
					...
					for (var l=0; l<PtList.length; l++) {
						...
						if (N != 'blank') {PtList[l][1] = N;}
						if (E != 'blank') {PtList[l][2] = E;}
						if (Z != 'blank') {PtList[l][3] = Z;}
						if (Desc) {PtList[l][4] = Desc;}
...
// Input:  The element's 'this' (optional - leave blank to have function return value)
...
// Audit lines and sketch resulting points
...
// Deletes the requested point(s) from the point summary text box
// Input:  Point list
...
// Check all point names in list against existing points matrix and delete rows when found
	...
	if (DeleteInput) {
		...
		$.prompt(qP(PtList.length + ' point' + Plural + ' will be deleted!<br><br>Delete point' + Plural + '?', 'POINT DATABASE EDIT'), {
...
// Renames the requested point(s) from the point summary text box
// Input:  Point list, start point number (optional)
// Output: The updated point list, or an alert box if the point list is invalid (through GetPointList)

// Get input values from form

	var PtList = qG('Delete').value; 
	var NextPt = qG('StartPt').value;
	var DupMessage = '';

// If no point suggested, try AutoPoint, then begin at 1 if no suggestions returned
	
	NextPt = NextPt ? NextPt : AutoPoint();
	NextPt = NextPt ? NextPt : 1 + '';
	
// Check to make sure field isn't blank

	if (!CheckBlank(PtList, ' list of points to rename', qG('Delete')))  return;
	
// Check validity of new point name, and handle duplicate point number by incrementing 

	if (!CheckPoint(NextPt, '', 0, 1))  return;
	
	if (CheckPoint(NextPt, '', 0, 1) == 'X') {
		
		DupMessage = 'Point ' + NextPt + ' is in use.<br><br>';
		
		NextPt = AutoPoint();
		
	}

// Get list of points to rename

	if (!(PtList = GetPointList(PtList, 1, 0, 0, qG('Delete'))))  return;

// Get point record matrix, lines record matrix

	var ptMt = ParsePoints(), linesMt = ParseLines(), lP = ptMt.length-1, tempPt;
	
	var Plural = ((PtList.length > 1) ? 's' : '');

// Rename required points if user confirms, updating lines database also

	$.prompt(qP(DupMessage + PtList.length + ' point' + Plural + ' will be renamed, starting at ' + NextPt + '!<br><br>Rename point' + Plural + '?', 'POINT DATABASE EDIT'), {
...
// Edit point name(s) in points database
					
					for (var j=0; j<lP; j++) {if (ptMt[j][0] == PtList[i][0]) {break;}}
					tempPt = ptMt[j][0];
					ptMt[j][0] = NextPt;
					
// Edit point name(s) in lines database
					
					for (var k=0; k<linesMt.length; k++) {
						
						if (linesMt[k][2] == tempPt) {linesMt[k][2] = NextPt;}
						if (linesMt[k][3] == tempPt) {linesMt[k][3] = NextPt;}
						if (linesMt[k][4] == tempPt) {linesMt[k][4] = NextPt;}
						linesMt[k][0] = '' + linesMt[k][2] + '.' + linesMt[k][3] + '.' + linesMt[k][4];

					}

// Increment point names
				...
				qG('Delete').value = ''; qG('StartPt').value = ''; qF('Delete');
...
// Pin ties and traverse

	data += '1 5012.2243700000 6011.6351600000 349.9956400000 Old_IP\n';
	data += '2 5009.5225641232 6035.0340158327 349.7561300000 Old_IP\n';
	data += '3 5054.3544772406 6040.2397598391 352.8821300000 Old_IP\n';
	data += '4 5057.0632732629 6016.8349057064 350.5370000000 Old_IP\n';
	data += '5 5055.5220886274 6001.6109057628 348.7561300000 Old_IP\n';
	data += '101 5064.8929996189 6051.3267419548 350.9945700000 TH\n';
	data += '102 5064.2181827237 5997.7070612160 351.1594700000 TH\n';
	data += '103 4997.0460543939 6000.7424716362 350.8879500000 TH\n';
	data += '104 4996.3424280978 6038.2727862237 350.5321400000 TH\n';
	data += '105 5064.9138648666 6051.3405152710 350.9725400000 OTH_101\n';
	data += '106 5064.2545071837 5997.7145651419 351.1345600000 OTH_102\n'; 

// Calc points

	data += '501 5005.21537 5994.64016 0.00000 Calc\n';
	data += '502 4998.56874 6017.24004 0.00000 Calc\n';
	data += '503 5041.85840 6029.97153 0.00000 Calc\n';
	data += '504 5048.50502 6007.37165 0.00000 Calc\n';
	data += '505 5049.57149 5992.10066 0.00000 Calc\n';
	data += '506 5014.29197 5997.30959 0.00000 C_Center\n';
	data += '510 5046.12818 5968.77949 0.00000 Calc\n';
	data += '511 5004.46636 5967.85325 0.00000 Calc\n';
	data += '512 4989.82263 6046.97859 0.00000 Calc\n';
...
// Output: The sample lines, parsed and input into the lines record
	...
	if (t = FormLine('L', '505', '501', '-', '0')) {lineMt.push(t);}
	if (t = FormLine('L', '501', '502', '-', '0')) {lineMt.push(t);}
	if (t = FormLine('L', '502', '503', '-', '0')) {lineMt.push(t);}
	if (t = FormLine('L', '503', '504', '-', '0')) {lineMt.push(t);}
	if (t = FormLine('A', '505', '504', '506', '0')) {lineMt.push(t);}
	if (t = FormLine('L', '510', '505', '-', '0')) {lineMt.push(t);}
	if (t = FormLine('L', '511', '501', '-', '0')) {lineMt.push(t);}
	if (t = FormLine('L', '503', '513', '-', '0')) {lineMt.push(t);}
	if (t = FormLine('L', '502', '512', '-', '0')) {lineMt.push(t);}
	if (t = FormLine('L', '510', '511', '-', '0')) {lineMt.push(t);}
	if (t = FormLine('L', '513', '512', '-', '0')) {lineMt.push(t);}
...
// Output: The loaded data from the file, parsed and saved as a current job
	...
	var extension = fileToLoad.name.split('.').pop();
    ...
    fileReader.onload = function(fileLoadedEvent) {
		...
		if (extension == 'qcg') {ReadTextFile(textFromFileLoaded);}
		else if (extension == 'csv') {ReadCSVFile(textFromFileLoaded);}
		
		else {
			
			var LoadError = 'Unable to use file. <br><br>Choose a Q-Cogo (.qcg) file to load a previous Q-Cogo job.  <br><br>Choose a CSV (.csv) file in PNEZD format to load more points into the current job.'
			qError(qG('LoadFileLabel'), LoadError, 'FILE NAME ERROR', 'errormessage');
...
function ReadCSVFile(Text) {CheckAllPoints(Text, 0, 0, 0, qG('LoadFileLabel'), 1);}
	
// Reads a .csv local save file and saves it as data in the current job
// Input:  Contents of loaded text file
// Output: The stored data: points, with appropriate overwrite warnings as necessary, through CheckAllPoints function with merge flag set
...
function exportCSVCoordinates() {

// Simple CSV Export
// Input:  none
// Output: The exported points, downloaded through the cogoDownloadCSV function

// Adapted from code written for Q-Cogo by Adam J Nowak, https://adamjnowak.com and used with thanks
	...
	if (points.length <= 1) {
		...
		$.prompt(qP('No points to export!', 'EXPORT'), {	
				classes: {message: 'errormessage'},		
		});
		...
		for (var i = 0; i < points.length-1; i++) {
			...
			pointArray[i] = points[i][0] + ',' + points[i][1] + ',' + points[i][2] + ',' + points[i][3] + ',' + points[i][4];
			
		}
		
		var exportPoints = pointArray.join('\n');
		var filename = $('#filename').val() + '.csv';
		
		cogoDownloadCSV(exportPoints, filename, 'text/csv;encoding:utf-8');
...
function cogoDownloadCSV(content, fileName, mimeType) {

// CSV Download

// Written for Q-Cogo by Adam J Nowak, https://adamjnowak.com and used with thanks
	...
	else {location.href = 'data:application/octet-stream,' + encodeURIComponent(content);}
