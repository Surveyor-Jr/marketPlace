/*
	...
	Use:	To provide sketching operations for Q-Cogo, <https://www.q-cogo.com/>.
	Ver:	1.7
...
function ExpandSketch(forceClose) {
	
// Expands and contracts the sketch when button pushed
// Input:  Force close flag (optional)
// Output: The expanded or contracted sketch area

// In both cases, refresh sketches after 0.15s CSS animation complete
	
	if ($('#PointsEasel').hasClass('bigsketch')) {
		
		$('#PointsEasel').removeClass('bigsketch');
		$('#ExpandLink').html('<i class="fa fa-expand fa-lg"></i>').prop('title', 'Expand Sketch');
		
		setTimeout(function(){RefreshSketches();  ZoomSketch(0, 0, 0);}, 250);
		
	}
	
	else if (!forceClose) {
		
		$('#PointsEasel').addClass('bigsketch');
		$('#ExpandLink').html('<i class="fa fa-compress fa-lg"></i>').prop('title', 'Compress Sketch');
...
// Sketch all points and labels (scenery points and labels in muted colour) and dashed line / curve between sequential points
		...
		for (i=0; i<lP2; i++) {
			...
			if (ptMt2[i][0].search(/\*/) < 0 &&  ptMt2[i + 1][0].search(/\*/) < 0)  {pntsCode += qLine(ptMt2[i], ptMt2[i + 1], Canvas, '#000099', 'Dotted');}

			else if (ptMt2[i][0].search(/\*/) >= 0) {

				var hwC = Math.abs(parseInt(cM[indexC][2]) - parseInt(cM[indexC + 1][2]));
				pntsCode += qArc(parseInt(cM[indexC][2]) + hwC/2, parseInt(cM[indexC][1]) + hwC/2, hwC/2, SEang[indexC], SEang[indexC + 1], Canvas,  '#000099', 'Dotted');
				indexC += 2;
...
// If point NOT stored, make diminished matrix of points, excluding calc point

	var dimMatrix;

	if (ptMt[3][0] == 'Calc_Pt') {dimMatrix = [ptMt[0], ptMt[1], ptMt[2], ptMt[4]];}
	else {dimMatrix = ptMt;}
...
// Sketch all points and labels (scenery points and labels in muted colour) and lines between appropriate points
	...
	pntsCode += Labels(Canvas, dimMatrix, lD, '#000099');
...
// If provided ID, make lines clickable with second transparent line
		...
		if (LineID) {
				...
				LineCode += 'onmouseover="evt.target.setAttribute(\'stroke-opacity\', \'0.25\');" ';
...
// If curve is within canvas extents, draw curve and return
	...
	if (Xc - R > 0 && Yc - R > 0 && Xc + R < WcReal && Yc + R < HcReal)  {
		...
		if (LineID) {
			...
			arcCode += 'onmouseover="evt.target.setAttribute(\'stroke-opacity\', \'0.25\');" ';
...
// This reads data as a linear chain -- could be improved to separate data into lines first
...
// Gets a line by ID, and returns it as a 1-dimensional vector
// Input:  Line ID to find
// Output: The appropriate row of the lines matix, or 0 if ID not found
	...
	for (var i=0; i<linesMt.length; i++) {
		...
		if (linesMt[i][0] == ID) {lineMatch = linesMt[i];}
...
// If points exist but distances unequal in arc, delete arc

		if (Fpt1 && Fpt2 && lType == 'A' && Fpt3) {

			var rad1 = FormatDecimal(PtPtInverse(0, '' + pt1, '' + pt3, 0, 0));
			var rad2 = FormatDecimal(PtPtInverse(0, '' + pt2, '' + pt3, 0, 0));
			
			if (rad1 != rad2) {linesMt.splice(k, 1); k--;}
...
// Input point at cursor if appropriate input field was previously focused
...
// Otherwise, display info about point
		...
		if (!isSelected('DeleteLink')) {
			...
			var ptVals = GetPointList(ptID, 0, 0, 1, 0);			
			CADinfo(ptVals[0]+ ': ' + FormatDecimal(ptVals[1]) + ', ' + FormatDecimal(ptVals[2]) + ', ' + FormatDecimal(ptVals[3]) + ' (' + ptVals[4] + ')');
...
// Output: Appropriate action or function call
	...
	if (isSelected('DeleteLink')) {
		...
		var thisLine = GetLine(lineID);
		
		if (thisLine) {
			
			if (thisLine[1] == 'L') {
				
				var invResults = PtPtInverse(0, thisLine[2], thisLine[3]);
				CADinfo(thisLine[2] + ' to ' + thisLine[3] + ': ' + FormatDecimal(invResults[0]) + ' ' + FormatDMS(invResults[2]));	
				
			}
			
			if (thisLine[1] == 'A') {
				
				var invR1 = PtPtInverse(0, thisLine[2], thisLine[4]);
				var invR2 = PtPtInverse(0, thisLine[3], thisLine[4]);
				
				var delta = invR2[2]-invR1[2];
				delta = delta < 0 ? delta + 2 * Math.PI : delta;
				
				CADinfo(thisLine[2] + ' to ' + thisLine[3] + ' (c=' + thisLine[4] + '): r=' + FormatDecimal(invR1[0]).trim() + ' a=' + FormatDecimal(invR1[0] * delta).trim() + ' d=' + FormatDMS(delta).trim());
...
// If no tool ID, clear and exit
	...
	if (!toolID) {clearAllCAD();  return;}

// If tool selected already, deselect all

	if (isSelected(toolID)) {clearAllCAD();}

// If not, deselect all, select

	else {clearAllCAD();  $('#' + toolID).addClass('button-selected');}
...
// Output: All lines and arcs in database, added to drawing
	...
	var lColour = Background ? '#999' : '#54751a';
...
// Updates the message to the user in the sketch screen
// Input:  Point to find, point matix and length
// Output: The requested PNEZD point vector
