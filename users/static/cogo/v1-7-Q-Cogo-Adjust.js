...
function AdjustSwitch(Store) {
	
// Triggers appropriate adjust function based on current header state
// Input:  Store point flag
// Output: The appropriate called adjust function

	var Title = qG('AdjustTitle').innerHTML;
	if (Title == 'Vertical Balance') {VerticalBalance(Store);}
	else if (Title == 'Compass Rule') {CompassRule(Store);}

}

// *******************************************************************************************************************************

function VerticalBalance(Store) {
	
// Solves and adjusts a traverse vertically
// Input:  Store point flag
// Output: The solved traverse, along with the adjusted points

	var N1 = '', E1 = '', StoreMessage = '', LevDet = qG('TolDetail').value;
	var AdjustHeader = 'ADJUST INPUT', AdjustHeader2 = 'ADJUST RESULTS';


	
// --------------------------------------------------
//  VB Section 1: Initial Checks, Prep Data
// --------------------------------------------------
...
// Sideshot matrix requires 2 companion vectors to house the "From" and "BS" point names for later reconstruction
	...
	if (SideList && !(SideList = GetPointList(SideList, 1, 1)))  return;
...
// --------------------------------------------------
//  VB Section 2: Vertical Closure
// --------------------------------------------------

// If there is more than one start or close point, form point vectors adjusted between Start2 and Close1 only

	var StartList2 = NonMutableArray(StartList), CloseList2 = NonMutableArray(CloseList);

	if (lS > 1 || lC > 1) {N1 = 'NOTE: Adjustment will be calculated between last Start point and first Close point only';}
	
	if (lS > 1) {StartList2.shift();}
	if (lC > 1) {CloseList2.pop();}
	
// Perform vertical closure checks - if above bounds, prompt and skip remainder of adjustment

	var Vmisc, CloseElev, ObsElev;
	
	CloseElev = CloseList2[0][3];
	if (lC > 1) {ObsElev = TravList[lTL-2][3]}  else {ObsElev = TravList[lTL-1][3]}
	Vmisc =  ObsElev - CloseElev;

	if (Math.abs(Vmisc) > qG('TolMisc').value) {
	
		E1 = 'Vertical misclosure does not fall within selected tolerance';
		qError(qG('AdjustPts'), E1, AdjustHeader2, 'errormessage');
...
// --------------------------------------------------
//  VB Section 3: Vertical Adjustment
// --------------------------------------------------
	
	if (!E1) {
	
		var TravTemp = TravList;
			
		var TravFinal = NonMutableArray(TravTemp);
		
		var Vcorr = Vmisc * -1;
		
// Form matrix of adjusted observation values (Point, Line Weight, dZ) for main traverse points
...
// Form matrix of total corrections
		...
		var totalZ = 0, TravApplied = new Array();

		for (var i=0; i<lTL; i++) {
		
			var tempIndex = ((i == lTL-1 && lC > 1) ? i-1 : i);
			var tempDZ = TravAdjust[tempIndex][3];
			if (!(i == lTL-1 && lC > 1)) {totalZ += tempDZ;}
		
			TravApplied[i] = [TravTemp[i][0], TravTemp[i][1], TravTemp[i][2], totalZ, TravTemp[i][4]];
		
		}

// Apply matrix of total corrections to arrive at final adjusted results

		for (var i=0; i<lTL; i++) {TravFinal[i][3] += TravApplied[i][3];}
			

	
// --------------------------------------------------
//  VB Section 4: Sideshots, Final Differencing
// --------------------------------------------------

// Compute matrix of sideshot observations and associated corrections

		var SideObs = new Array(), SSFrom, SSThis, SSCorr;
		...
		for (var i=0; i<lSL; i++) {
			...
			for (var j=0; j<lSL; j++) {if (TravApplied[j][0] == SSFrom) {break;}}
			
			if (TravApplied[j][3]) {SSCorr = TravApplied[j][3];} else {SSCorr = 0;}
		
			SideObs[i] = [SSFrom, SSThis, SSCorr];
		...
		for (var i=0; i<lSL; i++) {
			...
			SideFinal[i] = [SideList[i][0].replace('*', ''), SideList[i][1], SideList[i][2], parseFloat(SideList[i][3]) + parseFloat(SideObs[i][2]), SideList[i][4]];
...
// Form summary matrix of adjustment amounts and final coordinates
		...
		for (var i=0; i<lTL; i++) {
			...
			Summary[i] = [TravFinal[i][0], TravFinal[i][3]-TravList[i][3], TravFinal[i][3]];
		
		}
		
		for (var i=0; i<lSL; i++) {
		
			Summary[i + lTL] = [SideFinal[i][0], SideFinal[i][3]-SideList[i][3], SideFinal[i][3]];
...
// Prompt and overwrite points
			...
			if (Store) {
				...
				Message += '<span class="par-label">Misc:</span>' + FormatDecimal(Vmisc, 0, 1).trim() + '<br><br>';
...
// Update points matrix with new values
							...
							for (var m=0; m<ptMt.length; m++) {
								...
								for (var l=0; l<TravStore.length; l++) {
									...
									if (ptMt[m][0] == TravStore[l][0]) {ptMt[m][3] = TravStore[l][3];}

								}
								
								for (var k=0; k<SideStore.length; k++) {

									if (ptMt[m][0] == SideStore[k][0]) {ptMt[m][3] = SideStore[k][3];}
...
// Prepare results
	...
	var StartString2 = FormatPtList(StartList2, 0);
	var CloseString2 = FormatPtList(CloseList2, 0);
	var AdjString = FormatPtList(AdjList, 0);
	
	var Results = ''; 
	
	Results += OutputTitle('VERTICAL SUMMARY');
	...
	if (N1) {
		...
		Results += N1 + '\n';
		Results += '(Starting on ' + StartString2 + ' and Closing on ' + CloseString2 + ')' + '\n\n';
		
	}

	Results += '   Main Traverse Points: ' + lTL + '\n';
	Results += '   Sideshots:            ' + lSL + '\n\n';
	
	Results += 'Vertical Misclosure:\n\n';
	Results += '   Misclosure: ' + FormatDecimal(Vmisc, 0, 1).trim() + '\n\n';

// Prepare vertical adjustment results
	
	if (!E1) {
	
		Results += OutputTitle('VERTICAL ADJUSTMENT');
		
		Results += 'Main Traverse, Vertical Adjustment Parameters:' + '\n\n';
		Results += '   Vertical Correction: ' + FormatDecimal(Vcorr, 0, 1).trim() + '\n\n';

		if (LevDet > 1) {Results += OutputArray('Main Traverse, Line Weights and Vertical Corrections:', ['Point', 'Dist', 'Weight', 'dZ'], ['S', 'Dec', 'Dec', 'Dec'], TravAdjust);}

		if (LevDet > 1) {if (lC > 1) {Results +=  '   -> Both close points receive identical adjustments\n\n';}}
		
		if (LevDet > 1) {Results += OutputArray('Main Traverse, Total Applied Corrections:', ['Point', 'N', 'E', 'dZ', 'Desc'], ['S', 'Dec', 'Dec', 'Dec', 'S'], TravApplied);}

		if (LevDet > 1) {Results += OutputArray('Main Traverse, Final Adjusted Values:', ['Point', 'N', 'E', 'Z', 'Desc'], ['S', 'Dec', 'Dec', 'Dec', 'S'], TravFinal);}

// Prepare sideshot results

		if (LevDet > 1) {Results += OutputTitle('SIDESHOTS');}

		if (LevDet > 1) {Results += OutputArray('Sideshot Observations and Corrections:', ['From', 'To', 'dZ'], ['S', 'S', 'Dec'], SideObs);}
...
// Prepare final results
		...
		Results += OutputArray('Final Results Comparison:', ['Point', 'dZ', 'Z'], ['S', 'Dec', 'Dec'], Summary);
...
// --------------------------------------------------
//  CR Section 1: Initial Checks, Prep Data
// --------------------------------------------------
...
// If angle balance triggered, check angle close - if above bounds, prompt and skip remainder of adjustment
		...
		if (Math.abs(AdjEach) > ParseDMS(qG('TolAngle').value)) {
			...
			qError(qG('AdjustPts'), E2, AdjustHeader2, 'errormessage');
...
// --------------------------------------------------
//  CR Section 5: Sideshots, Final Differencing
// --------------------------------------------------
...
// Prepare angle balance results if applicable
